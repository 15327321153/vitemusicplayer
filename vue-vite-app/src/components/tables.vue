<template>
  <el-card class="box-card">
    ...
    <div class="table">
      ...
      <div class="tbody">
        <div class="titem" v-for="(item,index) in store.getlist()" :class="index%2?'bc':''" :key="index">
          <div class="sortitem txt">{{ store.getindexs('get')*20+index+1 }}</div>
      ...
      <div class="tfoot">
        ...
        <div class="tfootindex" v-for="(item,index) in store.getnumber()" :key="index" @click="change(index)" :class="index === store.getindexs('get')?'xuanzhong':''">{{ index+1 }}</div>
...
const altered = (str:string)=>{
  ...
  }else if(str === 'right' && store.getindexs("get") != store.getnumber()-1){
    store.getindexs("add")
  }
}
